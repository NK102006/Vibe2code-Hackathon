...
// Database Schema Initialization Middleware for Serverless Environments
...
async function ensureSchemaInitialized() {
  ...
  if (!schemaInitializationPromise) {
    schemaInitializationPromise = db.initializeSchema()
      .then(() => {
        schemaInitialized = true;
        console.log('Database tables successfully initialized.');
      })
      .catch(err => {
        schemaInitializationPromise = null; // Reset promise to retry on next request if failed
        throw err;
      });
  }
...
// Intercept requests to ensure database tables are set up
app.use(async (req, res, next) => {
  try {
    await ensureSchemaInitialized();
    next();
  } catch (err) {
    console.error('Database tables initialization error:', err);
    res.status(500).json({ error: 'Database tables initialization error: ' + err.message });
  }
});
